...
from PySide6.QtGui import QColor, QContextMenuEvent, QCursor, QFont, QImage, QKeyEvent, QMouseEvent, QPainter, QPaintEvent, QPalette, QPen, QPixmap, QTextOption
...
class ThumbnailGridView(QAbstractScrollArea):
    ...
    def set_column_count(self, columns: int) -> None:
        self._columns = max(2, min(4, columns))
        self._recalculate_metrics()
        self._update_scrollbar()
        self.viewport().update()
        self._schedule_visible_thumbnail_requests(immediate=True)
    ...
    def selected_indexes(self) -> list[int]:
        return sorted(index for index in self._selected_indexes if 0 <= index < len(self._items))
    ...
    def mouseDoubleClickEvent(self, event: QMouseEvent) -> None:
        if event.button() == Qt.MouseButton.LeftButton:
            ...
            if index >= 0:
                rect = self._item_rect(index)
                record = self._items[index]
                if self._left_arrow_rect(rect, record).contains(point) or self._right_arrow_rect(rect, record).contains(point):
                    event.accept()
                    return
    ...
    def mouseMoveEvent(self, event: QMouseEvent) -> None:
        ...
        if (
            ...
            pointer = hovered_winner >= 0 or hovered_reject >= 0 or hovered_left_arrow >= 0 or hovered_right_arrow >= 0
    ...
    def leaveEvent(self, event) -> None:
        ...
        for tile_index in {previous_winner, previous_reject, previous_left_arrow, previous_right_arrow}:
            if tile_index >= 0:
                self.viewport().update(self._item_rect(tile_index))
    ...
    def keyPressEvent(self, event: QKeyEvent) -> None:
        ...
        index = self._current_index if self._current_index >= 0 else 0
        key = event.key()
        ...
        if key == Qt.Key.Key_Left:
            self._set_single_selection(max(0, index - 1))
            return
        if key == Qt.Key.Key_Right:
            self._set_single_selection(min(len(self._items) - 1, index + 1))
            return
        if key == Qt.Key.Key_Up:
            self._set_single_selection(max(0, index - self._columns))
            return
        if key == Qt.Key.Key_Down:
            self._set_single_selection(min(len(self._items) - 1, index + self._columns))
            return
        if key == Qt.Key.Key_Home:
            self._set_single_selection(0)
            return
        if key == Qt.Key.Key_End:
            self._set_single_selection(len(self._items) - 1)
            return
        if key == Qt.Key.Key_PageUp:
            self._set_single_selection(max(0, index - page_step))
            return
        if key == Qt.Key.Key_PageDown:
            self._set_single_selection(min(len(self._items) - 1, index + page_step))
            return
    ...
    def _paint_tile(self, painter: QPainter, index: int, rect: QRect, record: ImageRecord, pixmap: QPixmap | None) -> None:
        ...
        if is_rejected:
            self._paint_state_badge(
                painter,
                QRect(image_rect.left() + 10, image_rect.top() + 10, 88, 24),
                "Rejected",
    ...
    def _select_range(self, index: int) -> None:
        ...
        self._selected_indexes = set(range(start, end + 1))
        self._set_current_index(index)
    ...
    def _select_all(self) -> None:
        if not self._items:
            return
        previous_selection = set(self._selected_indexes)
        self._selected_indexes = set(range(len(self._items)))
    ...
    def _visible_indexes(self) -> list[int]:
        ...
        end = min(len(self._items), (end_row + 1) * self._columns)
        return list(range(start, end))

    def _row_count(self) -> int:
        if not self._items:
            return 0
        return math.ceil(len(self._items) / self._columns)
    ...
    def _recalculate_metrics(self) -> None:
        ...
        self._tile_width_value = max(220, available // self._columns)
        self._image_height_value = max(180, int(self._tile_width_value * 0.72))
